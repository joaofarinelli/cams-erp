"""ffprobe + first-frame extraction helpers for the camera onboarding wizard."""
...
async def probe_stream(rtsp_url: str, timeout: float = 8.0) -> dict[str, Any]:
    """Run ffprobe against an RTSP URL with TCP transport. Short timeout so
    the wizard doesn't hang on a wrong password / unreachable host."""
    args = [
        "ffprobe",
        "-v", "error",
        "-rtsp_transport", "tcp",
...
async def first_frame_jpeg(rtsp_url: str, timeout: float = 8.0) -> bytes | None:
    """Grab a single JPEG from the live stream so the wizard can preview it."""
    args = [
        "ffmpeg",
        "-loglevel", "error",
        "-rtsp_transport", "tcp",
